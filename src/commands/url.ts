 ...
 * DBX url command
 *
 * Retrieves and displays the database connection URI for a provisioned instance.
 */
...
export function registerUrlCommand(program: Command): void {
  program
    ...
    .description("Retrieve database connection URL for an instance")
    .option(
      "--show-password",
      "Display the full connection URI with plaintext password"
    )
    .action(
      async (environment?: string, options?: { showPassword?: boolean }) => {
        try {
          // Load configuration
          ...
          try {
            if (options?.showPassword) {
              // Show full URI with password (auto-detects engine type)
              uri = buildConnectionURIAuto(metadata, config.vps.host);
            } else {
              // Show URI with masked password (auto-detects engine type)
              uri = buildConnectionURIMaskedAuto(metadata, config.vps.host);
            }
          } catch (err) {
            console.error(
